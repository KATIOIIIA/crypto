...
unsigned _int64 Encryption(unsigned _int64 M, unsigned _int64 key[],bool EorD)
{
	...
	{	
		...
		unsigned _int64 B;
		// Если шифруем, то берем K1, K2, .., а если дешифруем, то K16, K15,..
		if (EorD) B = E ^(key[j]);
...
void generationKeys(unsigned _int64 key, unsigned _int64 keys[])
{
	// ----------------------------------------------------------------------
	// КЛЮЧ
	// вх.данные: key - ключ; keys[] - ключи для каждого цикла;
...
//	bool status = EncryptionFile("Григ утро.mp3","Григ утроEC.mp3",keys,true);
//	if (status) EncryptionFile("Григ утроEC.mp3","Григ утро2.mp3",keys,false);

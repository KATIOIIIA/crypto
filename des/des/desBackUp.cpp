...
unsigned _int64 Encryption(unsigned _int64 M, unsigned _int64 key, bool EncrOrDeEncr)
{
	printBin(M,64);
	...
	for (int j=0;j<1;j++)
	{
		// меняем местами для следущего этапа
		if (j!=0)
		{
			L=R;
			R=L1;
		...
		unsigned _int64 B = E ^ newKey(key,j,EncrOrDeEncr);printf("B: \n");  printBin(B,64);
		// ------------------------------
		// Работа с S - блоками
		// ------------------------------
	...
	Result=MIX((P<<32)|L1,64,64,tableIPBack);
	printf("Result: %d\n",Result); 
	printBin(Result,64);
	return Result;
}
...
{
	unsigned _int64 res=Encryption(3850827209241952967,6350001347007632106,true); 
	printf("-------------------\n");
	unsigned _int64 key=182;
